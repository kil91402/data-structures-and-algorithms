...
const houseSize = (arr) => {
  ...
  const sizesArray = Object.keys(sizes).map((house) => ({
    house: house,
    members: sizes[house],
  }));
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach((character) => {
    const { house, spouse } = character;
    const memberCount = sizes[house];

    if (spouse && !deceasedSpouses.includes(spouse)) {
      sizes[house]--;
    }

    if (!survivors.some((survivor) => survivor.house === house)) {
      survivors.push({ house, members: memberCount });
    }
  });
